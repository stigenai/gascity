...
| SESSION-RECON-013 | Bounded async-start shutdown cleanup | Destructive cleanup intent is armed only when the selected runtime route exposes atomic `StopIfInstanceToken` and the admitted→cleanup journal CAS completes inside the shutdown deadline. Unsupported routes, unavailable context-bound CAS, lost CAS, and deadline uncertainty preserve the admitted journal and exact runtime, emit cleanup-incomplete evidence, and never fall back to probe-then-name-stop. A completion after the bounded wait retains an unarmed journal; a successor adopts an exact-token live runtime, settles definitely absent runtime only at/after `not_before`, and preserves all other uncertainty. | `cmd/gc/async_start_cleanup_test.go`; `internal/runtime/hybrid/hybrid_test.go`; `internal/beads/conditional_writes_resolve_internal_test.go` |
...
| SESSION-RUNTIME-007 | Session-list runtime enrichment | Independent persisted session rows are runtime-enriched concurrently under a fixed worker bound. Every row receives the same complete runtime overlay as a single-session read, and the returned slice preserves persisted order. | `internal/session/enrich_infos_concurrency_test.go`; `internal/session/manager.go` (`EnrichInfos`) |
| SESSION-RUNTIME-007 | Herdr registry liveness fencing | A Herdr agent registry row is not sufficient proof of a live runtime. When its pane ID is present, liveness requires a confirmed busy pane; a missing pane or bare shell projects the runtime missing so reconciliation can replace it. Confirmed-missing and bare-shell registered panes are reaped and their sidecar bindings cleared so stale registry names cannot fence out replacements. Startup enumeration applies the same pane fence before adoption; pane-probe transport uncertainty preserves the status-derived fail-safe verdict. | `internal/runtime/herdr/liveness_test.go`; `internal/runtime/herdr/panebinding_provider_test.go`; `internal/runtime/herdr/provider.go` |
| SESSION-RUNTIME-008 | Runtime-observed transcript identity | Before a keyless session transcript lookup, the worker asks the runtime for its provider-native session ID and persists a confirmed value through the session-owned `PersistSessionKey` command. Herdr projects only a registered agent identity whose kind is `id`; an explicit sidecar value wins. The exact key then fences transcript discovery so same-workdir histories are not guessed. | `internal/runtime/herdr/panebinding_provider_test.go`; `internal/worker/handle_test.go`; `internal/worker/handle_history.go` |
