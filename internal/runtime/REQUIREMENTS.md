...
| RUNTIME-CONTRACT-006 | Kubernetes external-rig workspace separation | When a Kubernetes session's work directory is outside the controller city tree, the provider stages it at `/workspace/rig`, keeps the initialized city at `/workspace`, and remaps rig/store environment paths and launch commands to `/workspace/rig`. City initialization must never overwrite the external rig's `.beads` identity or leave controller-only `/data/rigs/...` paths inside the worker. Before releasing the workspace gate, initialization materializes authored local packs, installs locked remote imports, and fails startup if either step fails so `gc hook` cannot start against a partial city. | `internal/runtime/k8s/pod.go`; `internal/runtime/k8s/staging.go`; `internal/runtime/k8s/provider.go`; `internal/runtime/k8s/provider_test.go` `TestProjectedPodExternalRigSeparatesCityAndLedgerPaths`, `TestInitCityInPodMaterializesPacksAndImports`; `internal/runtime/k8s/staging_test.go` `TestStageFilesKeepsExternalRigOutsideCityWorkspace` |
