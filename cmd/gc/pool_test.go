...
func TestDeepCopyAgentCoversAllFields(t *testing.T) {
	...
	src := config.Agent{
		...
		SecretReferences: []config.SecretReference{{
			ID: "auth", Environment: "AUTH_TOKEN",
			Kubernetes: &config.KubernetesSecretKeyReference{Name: "auth", Key: "token"},
			SSH:        &config.SSHSecretPathReference{Path: "/srv/secrets/auth"},
		}},
